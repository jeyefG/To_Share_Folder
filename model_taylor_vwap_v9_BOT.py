...
try:
    while True:
        ...
        for symbol in symbols:
            ...
            df_live = df_live[df_live['tick_volume'] > 0]
            bool_cols = df_live.select_dtypes(include='bool').columns
            df_live[bool_cols] = df_live[bool_cols].astype(int)
            ultima_fila = df_live.sort_values('time').iloc[[-1]]

            for etiqueta in etiquetas:
                modelo = trainer.modelos_por_symbol.get(symbol, {}).get(etiqueta)
                if modelo is None:
                    continue

                features = trainer._seleccionar_features(etiqueta)
                X_pred = ultima_fila[features]
                prob = modelo.predict_proba(X_pred)[:, 1][0]
