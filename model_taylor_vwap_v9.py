...
class FeatureEnricher:
    ...
    def enriquecer_historico(self, df):
        ...
        for symbol, data in df.groupby('symbol'):
            ...
            data['rsi_ema'] = data['rsi'].ewm(span=14, adjust=False).mean()
    ...
    def aplicar_clusters(self, df, features=None, n_clusters=3):
        """Aplica KMeans para asignar un cluster a cada fila."""
        if features is None:
            features = ['hora_normalizada',
                        'dist_to_sell_high',
                        'dist_to_vwap',
                        'open',
                        'dist_to_buy_high',
        ...
        for symbol, data in df.groupby('symbol'):
            ...
            data = data.copy()
            data['cluster'] = kmeans.fit_predict(datos_scaled)
            df_list.append(data)
...
class ModelTrainer:
    ...
    def _tune_model(self, X_train, y_train):
        """Realiza una búsqueda de hiperparámetros simple con GridSearchCV."""
        param_grid = {
            'num_leaves': [31, 63],
            'max_depth': [-1, 7, 15],
        ...
        base_model = LGBMClassifier(random_state=42, class_weight='balanced', verbose=-1)
    ...
    def entrenar_todos(self, verbose=True, plot=True, tune=False):
        for etiqueta in self.etiquetas_objetivo:
            if verbose:
...
if __name__ == "__main__":
    ...
    if df_total:
        ...
        df_concat = enricher.aplicar_clusters(df_concat)
...
trainer.entrenar_por_symbol(verbose=False, plot=False, tune=True)
